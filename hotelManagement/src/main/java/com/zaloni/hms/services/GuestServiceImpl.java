...
public class GuestServiceImpl implements GuestService{
	...
	public void sendBillByMail(Guest guest, String email){
		Room room;
		if(guest.getIsCheckedout()) {
		room = guest.getLastCheckedoutRoom();
		}else {
		...
		to.add(email);
		/*
		 * EmailMessage wiadomosc = new
		 * EmailMessage.EmailBuilder("marek.czwartek@wp.pl", to)
		 * .addSubject("Ticket purchase confirmation " + guest.getFirstName() + " " +
		 * guest.getLastName()) .addContent(bill) .build();
		 * 
		 * 
